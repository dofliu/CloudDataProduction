...
> 進度 Progress: **~100%**(P0–P4 + 上線硬化 + 產業庫擴充 + 課堂即時練習 + 學生監控台 + 4D 暖色 UI 重設計 + 3D 動畫精準化與自動驗證 + 學生可寫輸入控制 + 引擎產線物料流 + 學生決策層 / 九關 / 課堂即時互動 / 跨公司供應鏈 + **離線教材資料包**)· 更新 Updated: 2026-08-22
...
> CI 綠燈涵蓋 場景健全性(109 廠 / 259 台逐廠逐台)+ 12 支 Python 契約測試 + 前端建置 + 動畫一致性 53 項。
...
| **手工具後段 5 機種(2026-08-22)** | 補完「手工具製程主要流程圖」的後兩段(加工表面處理 / 組裝檢驗)。產業型別庫 20 → **25 種**:**研磨拋光機**(砂輪磨耗是消耗品的病、集塵堵是通道的病、主軸軸承是機構的病 —— 三條都讓振動升,但**只有軸承會走到 fault**,練「振動升到底來自哪一條」)、**清洗乾燥機**(噴嘴堵 → 壓力**升**但流量**掉**,反直覺的組合;洗不乾淨要到電鍍站才看得出來,本站儀表一切正常 —— 跨站根因分析)、**電鍍線**(鍍層厚度照法拉第定律算,學生拿 `current_density × dwell_time` 可驗算;「厚度夠但孔隙多」是只看厚度會漏掉的一整類不良)、**零件組裝機**(壓入力 × 位移要**合著看**,同樣壓到 24 mm 力的曲線不同就代表壓錯了 —— 多變量勝過單變量的最直觀例子)、**扭力功能測試機**(**量測站不是加工站**:感測器漂移會讓良品被誤退,學生只看退回率會誤判成上游品質變差,對症是 `calibrate_sensor` —— 刻意設計的陷阱題)。維修手冊新增 `replace_bath_chemistry`(換槽液 8 h)與 `replenish_anode`(補掛陽極 3 h),16 個新元件全部對得到具體動作、無一掉進 overhaul。**連續機的模型單位**:清洗與電鍍是連續機,出料節拍 ≠ 單件停留時間,兩者各自開 tag(`transit_time` 90 s / `dwell_time` 720 s),節拍才是產線用的數字 —— 硬把停留時間當節拍會讓下游餓料,那是模型單位選錯(A 批熔煉爐同一課)。場景尾端附加課堂版 c72/c73 與示範版 x01-f6/f7,各一條 5 站產線;既有 71 廠 / 175 台 / 41 段供應鏈**零位移**。**補上 A 批刻意留下的供應鏈缺口**:c71 鍛造 → c72 研磨電鍍 → c73 組裝檢驗 —— 這條接得下去是因為它**真的是 1:1**(一支鍛胚 = 一支扳手,工件不分裂也不合併);c70 熔煉仍然不接(一籃熔湯 → 多支棒料是 1:N)。接鏈後實測 c72→c73 餓料 14.3 h/24 h,**不漂亮但誠實** —— 全園區 43 段的餓料中位數 6.1 h、最大 17.4 h,新鏈在既有分布內;根因是最上游感應加熱爐 22 s 節拍鎖住整條鏈,那是真實的瓶頸(TOC),正好給 T4 瓶頸分析當教材。驗證:38 項訊號方向全符合退化語意、11 種單一元件退化各自產出**專屬**不良型別(零交叉污染)、良率健康 0.98–0.996 / 退化 0.20–0.81、動畫一致性 48 → **53 項**。**順手修掉的既有缺陷**:①(NL 建廠)公司字尾判斷用 1 字切片,永遠不可能等於「公司」且漏掉「工廠 / 工業」——「沖壓工廠有 2 台沖床」在 main 上就會多算一台憑空設備;量詞只認 台/臺/套/部/支/座,「兩條電鍍線」被算成 1 條。②(測試)`verify_scenario` 的「整台凍住」判定只比頭尾兩個取樣點,週期性起停的設備只要剛好在頭尾都停著就被誤判;改成逐拍記錄整個觀測窗,已雙向驗證(正常零誤報、刻意釘死一台仍抓得到)|
...
| **取數介面升級(2026-08-22)** | 盤點 P2:資料落了地,但「查得到」不等於「查得動」。`/api/history` 原本一次只能問「一台設備的一支 tag」,做一條產線的跨設備相關分析要打幾十次 API 再自己對齊時間戳 —— 取數本身變成作業的主要難度。① **`/api/history` 升級**:`devices=` / `tags=` 逗號分隔多值、`bucket=` 時間桶降採樣(每桶給 **avg / min / max / count** 四個量,不只平均 —— 預測性維護看的是峰值,振動尖峰被小時平均一抹就正好抹掉要偵測的東西)、`shape=wide|long`(預設 wide:一列一個時間點、一欄一條序列,直接進 pandas / Excel)、`format=csv`(帶 BOM,Excel 直接開)。**既有單設備單 tag 呼叫零回歸**(舊參數組合走原路徑、回傳結構逐欄比對,CI 守著)。wide 的 `limit` 是**時間列數**不是總點數(否則多序列會被裁成參差的洞),序列在窗內完全沒資料時列進 `missing` 欄位而不是靜靜消失。② **唯讀 SQL 端點**:`GET /api/sql`(+ `/api/sql/tables` 給表結構),學生直接寫 SQL 撈 —— 這才對得上技術棧選 TimescaleDB 的初衷,而且 SQL 是可轉移的技能。**安全邊界不靠字串比對**:關鍵字黑名單只是第一層,真正的防線是資料庫自己拒絕寫入(SQLite `file:…?mode=ro` URI 連線 / PostgreSQL `READ ONLY` 交易 + `statement_timeout`);測試刻意**繞過**正則直接對唯讀連線送 DELETE,由 DB 回`attempt to write a readonly database`。可查的表只有 `telemetry` / `events` / `production` / `production_hourly` —— 健康度與故障元件名(ground-truth)本來就不在這幾張表裡。③ 三種後端(TimescaleDB / SQLite / 降級記憶體)實測**逐列一致**。學生範例 `student_kit/p6_fetch_data.py`(四種取法,設備與 tag 由 `/api/catalog` 現查不寫死)。CI:`tests/test_data_access.py` 21 項 |
...
**動畫的技法告一段落;機型則依教學需要補到 25 種(2026-08-22 完成)。**
25 種機型、圖鑑、L1/L2/L3 綁定契約與 53 項自動驗證都在
...
2026-08-21 完成,見 Done 表「資料鏈落地」)→ **✅ P2 取數介面**(多設備多 tag / 降採樣 /
CSV + 唯讀 SQL,2026-08-22 完成,見 Done 表「取數介面升級」)→ **P3 能耗歸屬與廠級彙總**。
上面三格「最刺眼的缺口」P1 都補掉了,查得動的問題 P2 補掉了;還沒補的是
物料流 / 供應鏈 / 工單的**歷史**(只有當下),以及能耗歸屬(T15)。
...
- **更多產業 template** —— 25 種(2026-08-19 補 AOI / 焊接 / 雷切 / 包裝;08-21 補鑄造 / 鍛造上游 5 種;08-22 補手工具後段 5 種)已涵蓋手工具製程全鏈與主要教學需求;後續可補廢水 / 環控、噴塗。

> 已完成而從待辦移除:真 LLM 建廠、熱載入補完、產業庫擴充(10→11→15→20→25 種)、整合建廠自動上線(A+B+C)。
