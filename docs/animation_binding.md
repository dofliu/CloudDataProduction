...
### 4.18 `melting_furnace` — 熔煉爐(2026-08-21 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 爐體傾轉 | `tilt_angle` (deg, 0 直立 → −45 出湯) | L1 | 直接當繞 Z 的旋轉角;引擎 `−45·sin(u·π)` 同一條參數式 |
| 熔湯液面高度 | `bath_level` (%, 0..100) | L1 | 液面 y = 1.15 + 1.5·(level/100) |
| 出湯節拍 | `melt_cycle_time` (s, 72→332 隨電極消耗) | L3 | 牆鐘週期 = `melt_cycle_time / multiplier`,夾住並標示 |
| 熔湯色溫 / 亮度 | `melt_temp` (°C, 1450 設定點) | L2 | 700→1500 °C 對應暗橙→白熾 |
| 爐殼顏色(暗紅) | `shell_temp` (°C, 95→225) | L2 | ★ 爐襯磨蝕的現場徵候:外壁越燙越紅 |
| 浮渣厚度 / 顏色 | `slag_ratio` (%, 0.6→8) | L2 | 熔湯表面浮渣變厚變暗(夾渣不良的前因) |
| 電極輝光閃爍 | `electrode_current` (A, 780 ± 震盪) | L2 | 電極磨耗 → 震盪幅度變大(不是均值偏移) |
| 出湯流 | `tilt_angle` > 8° | L1 | 傾轉夠大才看得到湯流 |
| 累積出湯籃數 | `tap_count` | L1 | |

### 4.19 `die_casting_machine` — 壓鑄機(2026-08-21 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 移動模板開合 | `clamping_force` (ton, 350 額定) | L2 | 開度 = 1 − 力/(額定×0.9);力滿 = 完全鎖模 |
| 射出衝頭前進 | `shot_speed` (m/s, > 0.3 才動) | L1 | 壓鑄的射出是很短的一瞬,速度是引擎給的門檻 |
| 循環節拍 | `cycle_time` (s, 65→87) | L3 | 牆鐘週期 = `cycle_time / multiplier`,夾住並標示 |
| 固定 / 移動模板輝光 | `die_temp_fixed` / `die_temp_moving` (°C) | L2 | ★ 熱疲勞 → 兩側溫差拉開(移動側偏熱) |
| 真空指示燈 | `vacuum_level` (mbar, 60→270) | L1 | > 150 轉紅 = 密封劣化(氣孔的前因) |
| 縮孔 / 氣孔讀數 | `shrinkage_rate` / `porosity_rate` (%) | L1 | 兩條品質線分別對應模具與真空 |
| 機台抖動 | `vibration_rms` | L2 | |
| 累積鑄件數 | `cast_count` | L1 | |

### 4.20 `induction_heater` — 感應加熱爐(2026-08-21 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 棒料沿軌前進 | (無位置 tag) | **L3** | 引擎沒給位置,畫面依 22 s 額定節拍本地重建並標倍率 —— 不假裝是 L1 |
| 出料棒料色溫 | `billet_temp_out` (°C, 1180 目標) | L1 | ★ 600→1220 °C 對應暗紅→白熾;溫度不足一眼看得出來 |
| 線圈輝光 | `coil_current` (A, 0→1500) | L2 | |
| 冷卻水管亮度 | `cooling_flow` (L/min, 95→52) | L2 | 結垢 → 流量掉 → 變暗 |
| 漏電警示燈 | `leakage_current` (mA, > 12 轉紅) | L1 | 絕緣劣化(會走到 fault) |
| 功因讀數 | `power_factor` (0.96→0.74) | L1 | 與漏電流同源(coil_insulation) |
| 累積棒料數 | `billet_count` | L1 | |

### 4.21 `forging_press` — 鍛造壓機(2026-08-21 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 滑塊位置 | `ram_position` (mm, 0 上死點 → −180 下死點) | L1 | `/50`;引擎 `−90·(1−cos 2πph)` 同一條參數式 |
| 鍛打節拍 | `stroke_rate` (spm, 5→3.6) | L3 | 牆鐘週期 = 12 s / multiplier,夾住並標示 |
| 滑塊偏擺 | `ram_deviation` (mm, 0.04→1.6) | L2 | 導軌磨耗 → 繞 Z 微傾(位移換成看得見的角度) |
| 鍛件色溫 | `billet_temp_in` (°C, 1175) | L1 | 上游感應加熱爐給的溫度 |
| 鍛件欠肉(變小) | `underfill_rate` (%, 0.4→9) | L2 | 模具磨耗 → 充填不足 |
| 下模輝光 | `die_temp` (°C, 355) | L2 | |
| 除鱗噴霧大小 | `descale_pressure` (bar, 180→76) | L2 | ★ 噴嘴堵 → 噴霧變弱(壓入氧化皮的前因) |
| 機台抖動 | `vibration_rms` | L2 | 撞擊機架微震 |
| 累積鍛件數 | `forge_count` | L1 | |

### 4.22 `trimming_press` — 毛胚整修機(2026-08-21 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 滑塊位置 | `slide_position` (mm, 0 → −90) | L1 | `/50`;引擎 `−45·(1−cos 2πph)` 同一條參數式 |
| 切邊節拍 | `cycle_time` (s, 9→12) | L3 | 牆鐘週期 = `cycle_time / multiplier`,夾住並標示 |
| 刀座輝光(切斷力) | `trim_force` (ton, 220→341) | L2 | ★ **先動**的指標:刀口鈍 → 力先升 |
| 工件邊緣毛刺 | `burr_height` (mm, 0.02→0.44;規格 0.15) | L2 | ★ **後動**的指標:力升一段時間後毛刺才超規 |
| 頂桿伸出量 | `ejector_stroke` (mm, 25→18) | L1 | 磨耗 → 頂不到位(變形不良的前因) |
| 飛邊落下 | 滑塊行程 > 75% | L1 | 切斷後飛邊掉進料箱 |
| 機台抖動 | `vibration_rms` | L2 | |
| 累積整修數 | `trim_count` | L1 | |


### 4.23 `grinding_polisher` — 研磨拋光機(2026-08-22 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 砂輪半徑 | `wheel_diameter` (mm, 350→260) | L1 | ★ 消耗品餘命**畫在幾何上**:輪子真的變小 |
| 砂輪轉速 | `spindle_rpm` (rpm, 2850) | L3 | `visualSpin` 夾到 ≤1.5 rev/s 並標倍率(直接畫會變閃爍實心盤) |
| 研磨節拍 / 進退刀 | `cycle_time` (s, 18→24) | L3 | 前 62% 相位砂輪貼著工件(與引擎 `_contact()` 同一條) |
| 火花量 | `grind_force` (N, 85→157) | L2 | 砂輪鈍 → 要壓更大的力 → 火花更旺 |
| 工件表面質感 | `surface_ra` (µm, 0.34→1.8;規格 0.80) | L2 | Ra 越大越暗沉粗糙 |
| 集塵氣流粒子 | `extraction_flow` (m³/h, 2400→1000) | L2 | ★ 風量掉 → 粒子稀疏(「通道被堵住」的可見證據) |
| 主軸過熱輝光 | `spindle_temp` (°C, 62→117) | L2 | 只有軸承那條病會讓它升 |
| 機台抖動 | `vibration_rms` | L2 | |
| 累積研磨數 | `ground_count` | L1 | |

**探針** `probe:wheel_rim` —— 砂輪外緣世界高度 ↔ `wheel_diameter`。

三條退化線都會讓振動升,但**只有 `spindle_bearing_wear` 會走到 fault**;
畫面刻意讓三者各有專屬徵候(輪徑 / 氣流 / 主軸輝光),學生才分得開。

---

### 4.24 `cleaning_dryer` — 清洗乾燥機(2026-08-22 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 網帶上工件位置 | `cycle_time` (s, 15→18) | L3 | 等間距連續前進,倍率標在角落 |
| 噴桿輝光 | `spray_pressure` (bar, 3.2→5.2) | L2 | ★ 噴嘴堵 → 壓力**升** |
| 噴霧錐大小 | `spray_flow` (L/min, 180→86) | L2 | ★ 同一個堵塞 → 流量**掉**(與壓力反向) |
| 槽液濁度 | `bath_conductivity` (µS/cm, 320→2770) | L2 | 換液指標 |
| 烘乾段輝光 + 蒸氣 | `dry_temp` (°C, 105→82) | L2 | 加熱器老化 → 輝光變弱 |
| 工件顏色濁度 | `residue_level` (mg/m², 0.35→5.9;規格 2.0) | L2 | 洗不乾淨的結果 |
| 累積清洗數 | `washed_count` | L1 | |

**探針** `probe:belt_exit` —— 出料端(工件離開烘乾區的位置)。

> **連續機,不是批次機**:噴淋區 / 風刀區 / 烘乾區**同時**都有工件,所以三區的訊號
> 只看 `running`,畫面**不做**「這段在洗、那段在烘」的相位切換 —— 引擎那邊也一樣。
> `cycle_time`(出料節拍 15 s)與 `transit_time`(單件穿越全機 90 s)是兩個不同的數字,
> 兩支都開放讀取。

---

### 4.25 `plating_line` — 電鍍線(2026-08-22 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 天車位置 | `cycle_time` (s, 12→15) | L3 | 沿槽列連續前進,標倍率 |
| 陽極板厚度 | `anode_mass` (kg, 120→35) | L1 | ★ 消耗品餘命畫在幾何上 |
| 掛具金屬光澤 | `coating_thickness` (µm, 9.3→7.1;規格 ≥8.0) | L2 | 越厚越亮 |
| 掛具霧面程度 | `porosity_count` (1/cm², 0.7→7.5;規格 3.5) | L2 | ★ 厚度夠但孔隙多 → 只看厚度會漏掉的不良 |
| 鍍液色濁 | `bath_ph` (4.4→5.75) | L2 | 老化 → 偏黃綠 |
| 析氫氣泡量 | `current_density` (A/dm², 3.9→2.5) | L2 | |
| 整流器輝光 | `rectifier_temp` (°C, 50→92) | L2 | |
| 整流器閃爍 | `rectifier_ripple` (%, 1.2→14) | L2 | 紋波 → 輝光不穩 |
| 累積鍍件數 | `plated_count` | L1 | |

**探針** `probe:hoist` —— 天車世界 X ↔ 節拍相位。

> **連續掛鍍線**:同時有多掛在不同槽裡,所以電氣訊號只看 `running`,不隨天車相位閃動。
> 鍍層厚度走**法拉第定律**,用的是 `dwell_time`(槽內通電 720 s)**不是** `cycle_time`
> (出料節拍 12 s)。兩支都開放讀取,拿錯會算出錯的答案 —— 這個坑是刻意留的。

---

### 4.26 `assembly_station` — 零件組裝機(2026-08-22 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 壓頭位置 | `press_depth` (mm, 0→24) | L1 | 引擎 smoothstep 參數式同一條;倍率 ≈1 時鎖 L1 |
| 工件下沉量 | `press_depth` | L1 | 位移守恆:壓頭走多少工件就沉多少 |
| 電動起子下降 + 旋轉 | `screw_torque` (N·m, 9.0→6.1;下限 7.2) | L1 | 有值的那段相位才作動 |
| 背蓋翹起 | `screw_torque` 低於下限的程度 | L2 | 鎖不緊的可見徵候 |
| 料軌零件疏密 | `feed_success` (%, 97→66) | L2 | ★ 確定性門檻(不用亂數,畫面才不亂閃) |
| 料倉料位 | `feeder_level` (%, 100→6 鋸齒) | L1 | 補料在資料上長什麼樣 |
| 累積組裝數 | `assembled_count` | L1 | |

**探針** `probe:press_head` —— 壓頭端面世界高度 ↔ `press_depth`。

教學重點:**`press_force` × `press_depth` 要合著看** —— 同樣壓到 24 mm,力的曲線不同
就代表壓錯了(零件歪 / 少墊片)。只看最終深度永遠看不出來。

---

### 4.27 `torque_tester` — 扭力功能測試機(2026-08-22 新增)

| 視覺元素 | 引擎欄位 | 等級 | 映射 |
|----------|----------|------|------|
| 錶針角度 | `applied_torque` (N·m, 0→62) | L1 | 240° 掃過 0–90 N·m 滿刻度 |
| 峰值指針 | `peak_torque` (N·m) | L1 | 儀器讀到的值(**含**感測器偏差) |
| 紅色偏差指針 | `sensor_bias` (N·m, 0→±6.4) | L1 | ★ 教學照妖鏡:「儀器說的」與「真的」差多少 |
| 允收帶紅 / 綠 | `peak_torque` 是否超出 62±3.1 | L1 | 判定的視覺化 |
| 夾爪開度 | `clamp_pressure` (bar, 42→26) | L2 | 夾不緊 |
| 工件突然轉一下 | `slip_events` (count) 每 +1 | L1 | ★ 打滑 =「量測沒抓穩」看得見 |
| 加載頭下壓 | 前 70% 相位 | L3 | 標倍率 |
| 累積測試數 | `tested_count` | L1 | |

**探針** `probe:needle` —— 錶針端點世界位置 ↔ `applied_torque`。

> 這是**量測站**不是加工站。它的陷阱:感測器漂移會讓**良品被退回**,學生若只看退回率
> 會誤判成上游品質變差 —— 對症是 `calibrate_sensor`,不是換件。`sensor_bias` 刻意公開,
> 讓學生先建立「量測系統本身也會壞」的直覺。
