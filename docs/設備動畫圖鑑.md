...
| 機型 | 機台情境截圖 | 產線佈局 | 綁定 tag | 動畫一致性檢查 |
|---|---|---|---|---|
| 25 種 | 52 張 | 12 張 | 113 條綁定 · 67 條退化線 | 58 項全過 |
...
每台機器的「動畫綁定」欄位標了三種等級(定義見 [animation_binding.md](animation_binding.md) §1)。
...
![CNC 加工中心 — 健康:刀尖沿刻字刀路走,火花細而金黃](images/device_atlas/cnc_machining_center_healthy.webp)

*健康:刀尖沿刻字刀路走,火花細而金黃*

![CNC 加工中心 — 軸承退化 + 刀鈍:機台抖動、火花變多變紅](images/device_atlas/cnc_machining_center_degraded.webp)

*軸承退化 + 刀鈍:機台抖動、火花變多變紅*

![CNC 加工中心 — 故障閂鎖:機構立即停轉、柱燈轉紅閃爍並冒煙](images/device_atlas/cnc_machining_center_fault.webp)

*故障閂鎖:機構立即停轉、柱燈轉紅閃爍並冒煙*
...
![沖壓機 — 健康:滑塊落到下死點,撞擊火花四濺](images/device_atlas/stamping_press_healthy.webp)

*健康:滑塊落到下死點,撞擊火花四濺*

![沖壓機 — 模具磨耗 + 潤滑不足:工件邊緣毛躁變暗、潤滑燈轉黃](images/device_atlas/stamping_press_degraded.webp)

*模具磨耗 + 潤滑不足:工件邊緣毛躁變暗、潤滑燈轉黃*

![沖壓機 — 教師停機(run_enable=0):滑塊停上死點、柱燈黃燈慢閃](images/device_atlas/stamping_press_stopped.webp)

*教師停機(run_enable=0):滑塊停上死點、柱燈黃燈慢閃*
...
#### 射出成型機 `injection_molding`

循環相位由射出壓力與鎖模力反推,不另跑一套計時。

![射出成型機 — 健康:射出段壓力峰值,可動模板閉合](images/device_atlas/injection_molding_healthy.webp)

*健康:射出段壓力峰值,可動模板閉合*

![射出成型機 — 螺桿磨耗:循環拉長、油溫輝光變強](images/device_atlas/injection_molding_degraded.webp)
...
![半導體製程腔體 — 健康:電漿穩定、晶圓貫通進出片](images/device_atlas/semi_process_chamber_healthy.webp)

*健康:電漿穩定、晶圓貫通進出片*

![半導體製程腔體 — 製程漂移:腔內微粒暴增(subtle fault,設備不會停)](images/device_atlas/semi_process_chamber_degraded.webp)

*製程漂移:腔內微粒暴增(subtle fault,設備不會停)*
...
#### 熱處理爐 `heat_treat_furnace`

爐溫到不到得了設定點,就是加熱元件老化的直接證據。

![熱處理爐 — 健康:爐溫到 897 °C,爐膛均勻亮橘](images/device_atlas/heat_treat_furnace_healthy.webp)

*健康:爐溫到 897 °C,爐膛均勻亮橘*

![熱處理爐 — 元件老化 + 洩漏:爐溫掉、熱斑不均、殘氧警示](images/device_atlas/heat_treat_furnace_degraded.webp)

*元件老化 + 洩漏:爐溫掉、熱斑不均、殘氧警示*
...
#### 焊接機器人工作站 `welding_cell`

電弧開關由 `arc_current` 判定 —— 遙測說有弧才畫弧,不猜相位。

![焊接機器人工作站 — 健康:電弧沿焊道前進,飛濺細少](images/device_atlas/welding_cell_healthy.webp)

*健康:電弧沿焊道前進,飛濺細少*

![焊接機器人工作站 — 噴嘴堵 + 送絲磨損:飛濺暴增、氣流量掉到 10 L/min](images/device_atlas/welding_cell_degraded.webp)

*噴嘴堵 + 送絲磨損:飛濺暴增、氣流量掉到 10 L/min*
...
![雷射切割機 — 健康:光束沿輪廓出光,切速 34.6 mm/s](images/device_atlas/laser_cutter_healthy.webp)

*健康:光束沿輪廓出光,切速 34.6 mm/s*

![雷射切割機 — 鏡片污損 + 冷卻劣化:鏡溫 88 °C、切速掉到 24.8](images/device_atlas/laser_cutter_degraded.webp)

*鏡片污損 + 冷卻劣化:鏡溫 88 °C、切速掉到 24.8*
...
#### AOI 光學檢測站 `aoi_inspection`

檢測站說不良,不代表工件真的不良 —— 量測系統本身也會劣化。

![AOI 光學檢測站 — 健康:focus 95、誤判率 0.7 %](images/device_atlas/aoi_inspection_healthy.webp)

*健康:focus 95、誤判率 0.7 %*

![AOI 光學檢測站 — 鏡頭污染 + 光源衰減:focus 掉到 66、誤判率 9.4 %](images/device_atlas/aoi_inspection_degraded.webp)

*鏡頭污染 + 光源衰減:focus 掉到 66、誤判率 9.4 %*
...
#### 包裝機 `packaging_machine`

產線終站:封口溫度到不了設定點,就是加熱器老化。

![包裝機 — 健康:封口鉗閉合、封口溫度 144.6 °C](images/device_atlas/packaging_machine_healthy.webp)

*健康:封口鉗閉合、封口溫度 144.6 °C*

![包裝機 — 加熱器老化:溫度只到 118 °C、不良率 7.8 %](images/device_atlas/packaging_machine_degraded.webp)

*加熱器老化:溫度只到 118 °C、不良率 7.8 %*
...
#### 六軸機械手臂 `robot_arm_6axis`

六軸角度全用,末端位置由引擎正運動學算出,畫面必須對得上。

![六軸機械手臂 — 健康:取件姿態,夾爪落在取料站上方](images/device_atlas/robot_arm_6axis_healthy.webp)

*健康:取件姿態,夾爪落在取料站上方*

![六軸機械手臂 — 減速機退化:機身抖動明顯](images/device_atlas/robot_arm_6axis_degraded.webp)
...
#### AGV 搬運車 `agv_mobile_robot`

車體補間走弧長鎖定,任何時刻都在巡迴路徑上,不切轉角。

![AGV 搬運車 — 健康:載貨移動中,車體恆在巡迴路線上](images/device_atlas/agv_mobile_robot_healthy.webp)

*健康:載貨移動中,車體恆在巡迴路線上*

![AGV 搬運車 — 停靠上料站:到位貼齊,與遙測座標零誤差](images/device_atlas/agv_mobile_robot_docked.webp)
...
![輸送帶 — 健康:皮帶等速捲動,工件依帳上件數前進](images/device_atlas/conveyor_healthy.webp)

*健康:皮帶等速捲動,工件依帳上件數前進*

![輸送帶 — 軸承退化:振動與馬達電流同步升高](images/device_atlas/conveyor_degraded.webp)
...
2026-08-21 依「手工具製程主要流程圖」補的前段。共同語彙:溫度是主角 —— 熔湯、棒料、鍛模的顏色就是製程狀態,而爐殼與模具的溫度則是「該保養了」的徵候。

#### 熔煉爐 `melting_furnace`

爐體傾轉出湯直接吃 `tilt_angle`;爐殼由灰轉暗紅就是「該重砌爐襯」。

![熔煉爐 — 健康:爐體傾轉出湯,熔湯白熾、爐殼仍是灰色](images/device_atlas/melting_furnace_healthy.webp)

*健康:爐體傾轉出湯,熔湯白熾、爐殼仍是灰色*

![熔煉爐 — 爐襯磨蝕 + 爐渣:爐殼轉暗紅、浮渣變厚、爐溫掉到 1387 °C](images/device_atlas/melting_furnace_degraded.webp)

*爐襯磨蝕 + 爐渣:爐殼轉暗紅、浮渣變厚、爐溫掉到 1387 °C*
...
#### 壓鑄機 `die_casting_machine`

兩側模溫分開上色 —— 溫差拉開就是模具熱疲勞,真空燈轉紅則是密封劣化。

![壓鑄機 — 健康:鎖模 347 ton、真空 67 mbar、兩側模溫幾乎相同](images/device_atlas/die_casting_machine_healthy.webp)

*健康:鎖模 347 ton、真空 67 mbar、兩側模溫幾乎相同*

![壓鑄機 — 熱疲勞 + 真空劣化:模溫差 37 °C、真空 232 mbar、縮孔與氣孔都破 6%](images/device_atlas/die_casting_machine_degraded.webp)

*熱疲勞 + 真空劣化:模溫差 37 °C、真空 232 mbar、縮孔與氣孔都破 6%*
...
#### 感應加熱爐 `induction_heater`

棒料位置是本地重建(引擎沒給位置 tag,標倍率不假裝 L1);出料色溫是 L1。

![感應加熱爐 — 健康:出料 1181 °C 白熾,漏電流 2 mA、功因 0.95](images/device_atlas/induction_heater_healthy.webp)

*健康:出料 1181 °C 白熾,漏電流 2 mA、功因 0.95*

![感應加熱爐 — 絕緣劣化 + 結垢:出料只剩 1072 °C(不足)、漏電流 39.7 mA](images/device_atlas/induction_heater_degraded.webp)

*絕緣劣化 + 結垢:出料只剩 1072 °C(不足)、漏電流 39.7 mA*
...
![鍛造壓機 — 健康:下死點 1520 ton 成形,鍛件紅熱、除鱗噴霧飽滿](images/device_atlas/forging_press_healthy.webp)

*健康:下死點 1520 ton 成形,鍛件紅熱、除鱗噴霧飽滿*

![鍛造壓機 — 鍛模磨耗 + 噴嘴堵:偏擺 1.42 mm、欠肉 7.6%、氧化皮 8.2%](images/device_atlas/forging_press_degraded.webp)

*鍛模磨耗 + 噴嘴堵:偏擺 1.42 mm、欠肉 7.6%、氧化皮 8.2%*
...
#### 毛胚整修機 `trimming_press`

刀口鈍化時**切斷力先升、殘毛刺後升** —— 兩個指標的時間差就是這台的教學重點。

![毛胚整修機 — 健康:切斷力 214 ton、毛刺 0.03 mm(規格內),飛邊落進料箱](images/device_atlas/trimming_press_healthy.webp)

*健康:切斷力 214 ton、毛刺 0.03 mm(規格內),飛邊落進料箱*

![毛胚整修機 — 刀口鈍化:切斷力 336 ton、毛刺 0.35 mm 超規、變形率 5.2%](images/device_atlas/trimming_press_degraded.webp)

*刀口鈍化:切斷力 336 ton、毛刺 0.35 mm 超規、變形率 5.2%*
...
2026-08-22 依「手工具製程主要流程圖」補的後兩段。共同教學重點:**不良的根因常常不在本站** —— 清洗沒洗乾淨要到電鍍站才看得出來,而扭力測試站的退回率上升,可能只是它自己的感測器該校正了。

#### 研磨拋光機 `grinding_polisher`

三條病都讓振動升,但只有主軸軸承會走到 fault —— 砂輪看輪徑、集塵看氣流,分得開。

![研磨拋光機 — 健康:Ra 0.35 µm、砂輪 Ø344、抽風 2312 m³/h](images/device_atlas/grinding_polisher_healthy.webp)

*健康:Ra 0.35 µm、砂輪 Ø344、抽風 2312 m³/h*

![研磨拋光機 — 砂輪磨耗 + 集塵堵:Ra 1.51 µm(規格 0.80)、輪徑剩 271、風量掉到 1268](images/device_atlas/grinding_polisher_degraded.webp)

*砂輪磨耗 + 集塵堵:Ra 1.51 µm(規格 0.80)、輪徑剩 271、風量掉到 1268*
...
#### 清洗乾燥機 `cleaning_dryer`

連續網帶機:三區同時有工件。噴嘴堵時**壓力升但流量掉** —— 反直覺的組合是本站的重點。

![清洗乾燥機 — 健康:殘留 0.44 mg/m²、壓力 3.19 bar / 流量 172 L/min 相稱](images/device_atlas/cleaning_dryer_healthy.webp)

*健康:殘留 0.44 mg/m²、壓力 3.19 bar / 流量 172 L/min 相稱*

![清洗乾燥機 — 噴嘴堵:壓力升到 4.86 但流量只剩 88 —— 一升一降就是堵塞的指紋](images/device_atlas/cleaning_dryer_degraded.webp)

*噴嘴堵:壓力升到 4.86 但流量只剩 88 —— 一升一降就是堵塞的指紋*
...
![電鍍線 — 健康:9.28 µm(規格 ≥8)、孔隙 0.81、陽極 114 kg](images/device_atlas/plating_line_healthy.webp)

*健康:9.28 µm(規格 ≥8)、孔隙 0.81、陽極 114 kg*

![電鍍線 — 陽極消耗 + 鍍液老化:7.09 µm 過薄、孔隙 4.86 破表、陽極剩 57 kg](images/device_atlas/plating_line_degraded.webp)

*陽極消耗 + 鍍液老化:7.09 µm 過薄、孔隙 4.86 破表、陽極剩 57 kg*
...
#### 零件組裝機 `assembly_station`

壓入力 × 位移**要合著看** —— 同樣壓到 24 mm,力的曲線不同就代表壓錯了。

![零件組裝機 — 健康:給料 97.2%、鎖付 8.94 N·m、缺件 0.26%](images/device_atlas/assembly_station_healthy.webp)

*健康:給料 97.2%、鎖付 8.94 N·m、缺件 0.26%*

![零件組裝機 — 卡料 + 起子衰退:給料剩 67.4%、鎖付 6.21(下限 7.2)、缺件 5.38%](images/device_atlas/assembly_station_degraded.webp)

*卡料 + 起子衰退:給料剩 67.4%、鎖付 6.21(下限 7.2)、缺件 5.38%*
...
![扭力測試機 — 健康:峰值 62.1 落在允收帶內、偏差 +0.22、打滑 6 次](images/device_atlas/torque_tester_healthy.webp)

*健康:峰值 62.1 落在允收帶內、偏差 +0.22、打滑 6 次*

![扭力測試機 — 感測器漂移 −4.61:峰值讀成 53.9 被判退 —— 但工件其實是好的](images/device_atlas/torque_tester_degraded.webp)

*感測器漂移 −4.61:峰值讀成 53.9 被判退 —— 但工件其實是好的*
...
![空壓機 — 健康:出口壓力貼著設定點紅線](images/device_atlas/air_compressor_healthy.webp)

*健康:出口壓力貼著設定點紅線*

![空壓機 — 濾網阻塞 + 軸承退化:電流升、風量掉](images/device_atlas/air_compressor_degraded.webp)
...
![智慧電表 — 健康:三相平衡、功因 0.95 以上](images/device_atlas/energy_meter_healthy.webp)

*健康:三相平衡、功因 0.95 以上*

![智慧電表 — 電容老化:功因下滑,三相長條看得出不平衡](images/device_atlas/energy_meter_degraded.webp)

*電容老化:功因下滑,三相長條看得出不平衡*
...
![風力發電機 — 健康:葉片工作角,穩定發電](images/device_atlas/wind_turbine_healthy.webp)

*健康:葉片工作角,穩定發電*

![風力發電機 — 教師停機:葉片順槳 88°,畫面標示 pitch 角](images/device_atlas/wind_turbine_stopped.webp)

*教師停機:葉片順槳 88°,畫面標示 pitch 角*
...
machine_tool / precision_parts 的主力配方

![CNC 加工 → 手臂取放 → 輸送帶出料](images/device_atlas/line_cnc.webp)

### 射出成型 → 手臂取放 → 輸送帶出料

plastics 配方;手臂取件點對到射出機出料側

![射出成型 → 手臂取放 → 輸送帶出料](images/device_atlas/line_inj.webp)

### 沖壓成形 → 手臂取放

metal_forming 配方,兩站直接交接

![沖壓成形 → 手臂取放](images/device_atlas/line_press.webp)

### 焊接接合 → 手臂取放 → 輸送帶出料

新產業 c66 的配方

![焊接接合 → 手臂取放 → 輸送帶出料](images/device_atlas/line_weld.webp)

### 雷射切割 → 手臂取放 → 包裝出貨

新產業 c67:包裝機當產線終站

![雷射切割 → 手臂取放 → 包裝出貨](images/device_atlas/line_laserpack.webp)

### 射出成型 → 手臂取放 → AOI 檢測

新產業 c68:全檢線

![射出成型 → 手臂取放 → AOI 檢測](images/device_atlas/line_aoi.webp)

### CNC 加工 → AGV 搬運 → 輸送帶出料

AGV compact 模式:巡迴路線縮尺 0.25 並畫縮小標線

![CNC 加工 → AGV 搬運 → 輸送帶出料](images/device_atlas/line_agv.webp)

### 熔煉出湯 → 手臂 → 壓鑄成形 → 手臂 → 輸送帶出料

c70 / x01-f4 的五站鑄造線:熔煉爐是全線瓶頸(72 s 一籃)

![熔煉出湯 → 手臂 → 壓鑄成形 → 手臂 → 輸送帶出料](images/device_atlas/line_casting.webp)

### 感應加熱 → 手臂 → 熱模鍛造 → 手臂 → 切邊整修

c71 / x01-f5 的五站鍛造線:出料溫度是第一個品質關卡

![感應加熱 → 手臂 → 熱模鍛造 → 手臂 → 切邊整修](images/device_atlas/line_forging.webp)

### 研磨拋光 → 手臂 → 清洗乾燥 → 手臂 → 電鍍表面

c72 / x01-f6 的五站表面處理線:洗不乾淨的工件,鍍層會附不住

![研磨拋光 → 手臂 → 清洗乾燥 → 手臂 → 電鍍表面](images/device_atlas/line_finishing.webp)

### 零件組裝 → 手臂 → 扭力測試 → 手臂 → 輸送帶出貨

c73 / x01-f7 的五站組裝檢驗線:成品逐支驗扭力才出貨

![零件組裝 → 手臂 → 扭力測試 → 手臂 → 輸送帶出貨](images/device_atlas/line_handtool.webp)

### 混合壓力測試

七台同場:驗證燈光只有一組、佈局不互相穿模

![混合壓力測試](images/device_atlas/line_mixed.webp)
