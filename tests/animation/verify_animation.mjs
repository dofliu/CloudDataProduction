...
console.log("\n[25] 研磨拋光機 · slow(×1)—— 砂輪外緣世界高度 ↔ wheel_diameter");
{
  ...
  // 探針掛在輪緣、隨自轉繞行,所以**不能**比世界 Y(那會隨轉角在 ±R 之間掃)。
  // 要比的是「輪心 → 探針」的距離 —— 那就是渲染出來的半徑,與轉到哪個角度無關。
  ...
  const MM = 1 / 50, HUB = { x: 0.5, y: 3.3, z: 0 }, DMAX = 350, DMIN = 260;
  const dev = rows.map((r) => {
    const p = r.probes.wheel_rim;
    const rad = Math.hypot(p.x - HUB.x, p.y - HUB.y, p.z - HUB.z);
    const d = Math.min(DMAX, Math.max(DMIN, r.tags.wheel_diameter));
    return Math.abs(rad - d * 0.5 * MM);          // 期望半徑 = 直徑/2 × MM
  });
  const maxDev = rows.length ? Math.max(...dev) : Infinity;
  const dias = rows.map((r) => r.tags.wheel_diameter);
  const span = rows.length ? Math.max(...dias) - Math.min(...dias) : 0;
  // 40 秒的擷取窗裡砂輪幾乎不會磨掉(輪徑變動 <1 mm),所以這項驗的是**常數映射對不對**
  // (MM 換算、輪心位置、探針有沒有跟著縮),不是驗斜率。另加一道「半徑落在合理帶內」
  // 的保險,擋掉「探針沒跟著縮放」或「輪子被畫成 0」這類錯法。
  const rads = rows.map((r) => Math.hypot(r.probes.wheel_rim.x - HUB.x,
                                          r.probes.wheel_rim.y - HUB.y,
                                          r.probes.wheel_rim.z - HUB.z));
  const rMin = Math.min(...rads), rMax = Math.max(...rads);
  const inBand = rMin >= DMIN * 0.5 * MM - 0.02 && rMax <= DMAX * 0.5 * MM + 0.02;
  check("研磨 wheel_diameter → 砂輪渲染半徑(輪心↔探針距離)",
        rows.length > 5 && maxDev <= 0.05 && inBand,
        `半徑誤差 max ${maxDev.toFixed(4)} 模型單位(容許 ≤0.05)· 實測半徑 ${rMin.toFixed(3)}–${rMax.toFixed(3)}`
        + `(合理帶 ${(DMIN * 0.5 * MM).toFixed(2)}–${(DMAX * 0.5 * MM).toFixed(2)})· 輪徑變動 ${span.toFixed(2)} mm`);
}
...
{
  // 天車是 **L3 自由播放**(相位吃牆鐘 delta,不吃 frame index),所以逐幀取樣拿到的
  // 相位本來就是任意的 —— 斷言「取樣之間單調前進」不是契約保證的事,只是碰運氣。
  // L3 真正該保證的是:①走遍導軌(不是卡住不動)②不衝出導軌範圍。
  ...
  const lo = -RAIL / 2 - 0.05, hi = RAIL / 2 + 0.05;
  const inRail = xs.every((x) => x >= lo && x <= hi);
  check("電鍍 天車沿導軌行走(行程涵蓋 + 不出軌)",
        rows.length > 8 && span > RAIL * 0.60 && inRail,
        `行程 ${span.toFixed(2)} / 導軌 ${RAIL.toFixed(1)}(需 >60%)· 全部 ${xs.length} 幀都在導軌內:${inRail}`);
}
