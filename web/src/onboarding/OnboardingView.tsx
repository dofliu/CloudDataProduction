...
export default function OnboardingView({
  ...
  return (
    <div className="catalog" style={{ maxWidth: 1080 }}>
      {/* Hero */}
      <div style={{ background: "linear-gradient(135deg,#f4e6d2,#f6efe2)", border: "1px solid var(--line)",
        ...
        {courseStatus?.current_week != null && (
          <div style={{ marginTop: 10, display: "inline-flex", alignItems: "center", gap: 8, fontSize: 13,
                        color: "#5a4c36", background: "#f4e6d2", border: "1px solid #e0d0b6",
                        borderRadius: 8, padding: "6px 12px" }}>
            📅 本週課程:<b style={{ color: "#b5622e" }}>第 {courseStatus.current_week} 週{courseStatus.title ? `「${courseStatus.title}」` : ""}</b>
            {` · 稼動率 ${courseStatus.utilization}`}
          </div>
        )}
        <p style={{ margin: "8px 0 0", color: "#5a4c36", lineHeight: 1.7, maxWidth: 760 }}>
          你是這座虛擬工業區的<b style={{ color: "#b5622e" }}>維運工程師</b>。園區裡 {companies.length} 間公司、
          數十台真實運轉的設備(以標準 <b>Modbus / OPC-UA / MQTT</b> 協定對外),
          會健康地跑、也會慢慢退化甚至故障。你的任務:<b style={{ color: "#453a29" }}>連上它們、監控它們、在壞掉前抓到徵兆</b>。
        </p>
        <div style={{ marginTop: 10, fontSize: 12, color: "#d47a3f" }}>
          ⚠ 全部為合成數據(synthetic),帶 ground-truth 標籤,專為教學設計 —— 放心動手、放心試錯。
        </div>
        {(onOpenTour || onOpenDemo) && (
          <div style={{ marginTop: 14, display: "flex", gap: 8, flexWrap: "wrap" }}>
            {onOpenTour && <button onClick={onOpenTour} style={btn("#b5622e")}>🎮 新手導覽(60 秒)</button>}
            {onOpenDemo && <button onClick={onOpenDemo} style={btn("#d47a3f")}>▶ 看完整範例(認領→故障→診斷→排除)</button>}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 6 }}>
        {lockedIdentity ? (
          <span className="hint" style={{ margin: 0 }}>登入為 <b style={{ color: "#5a9e5a" }}>{lockedIdentity}</b>(課堂計分、認領、繳交都用這個帳號)</span>
        ) : (
          <>
            ...
            <button onClick={saveMe} style={btn("#b5622e")}>設定</button>
            {me && <span style={{ color: "#5a9e5a", fontWeight: 600 }}>目前:{me}</span>}
          </>
        )}
        {msg && <span className="hint" style={{ margin: 0, color: "#b5622e" }}>· {msg}</span>}
      </div>

      {/* 任務進度 */}
      <h3 style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 10 }}>
        你的任務進度
        <span style={{ fontSize: 13, color: allDone ? "#5a9e5a" : "var(--muted)", fontWeight: 600 }}>
          {doneN} / {steps.length} {allDone ? "· 全部完成,你已是合格的園區維運工程師 🏅" : ""}
        </span>
      </h3>
      <div style={{ height: 8, background: "#f6efe2", border: "1px solid var(--line)", borderRadius: 6, overflow: "hidden", marginBottom: 12 }}>
        <div style={{ width: `${(doneN / steps.length) * 100}%`, height: "100%",
                      background: allDone ? "#5a9e5a" : "linear-gradient(90deg,#b5622e,#5a9e5a)", transition: "width .4s" }} />
      </div>
      <ol style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
        {steps.map((s, i) => (
          <li key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start",
                               border: `1px solid ${s.done ? "#bcd6a6" : "var(--line)"}`, borderRadius: 8,
                               padding: "10px 12px", background: s.done ? "#eef4e8" : "var(--panel)" }}>
            <div style={{ width: 26, height: 26, flex: "0 0 26px", borderRadius: 26, fontWeight: 700,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          background: s.done ? "#5a9e5a" : "#e0d0b6", color: s.done ? "#fffaf0" : "#8a7c63" }}>
              {s.done ? "✓" : i + 1}
            </div>
            <div>
              <div style={{ fontWeight: 600, color: s.done ? "#cfe6c0" : "#453a29" }}>{s.title}</div>
      ...
      {me && !myCompany && (
        <div style={card}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => claim(claimPick)} style={btn("#5a9e5a")}>認領</button>
        ...
        <div style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            {liveVal !== undefined && (
              <div style={{ fontSize: 13 }}>
                ...
                <b style={{ color: "#5a9e5a" }}>{kit.tag} = {liveVal.toFixed(2)} {kit.tagUnit}</b>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => copy(kitMode === "read" ? kit.python : kit.pythonMonitor, "python")}
                    style={{ ...btn("#b5622e"), position: "absolute", right: 8, top: 8, zIndex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 12, flexWrap: "wrap" }}>
            <button onClick={markConnected} disabled={connected} style={btn(connected ? "#bcd6a6" : "#5a9e5a")}>
      ...
      {myCompany && (
        <>
          ...
          {myFaulted.length > 0 && (
            <div style={{ margin: "0 0 10px", padding: "10px 14px", borderRadius: 8, background: "#f7e6e2",
                          border: "1px solid #eccabf", color: "#eccabf", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ⚠ 你有 {myFaulted.length} 台設備故障（{myFaulted.join("、")}）——
              到「學生面」ack 確認並 resolve 處置,完成任務 ④。
              <button onClick={() => onNav("student")} style={btn("#c85a4a", "#fff")}>去處置工單</button>
            </div>
          )}
            ...
            <div style={card}>
              ...
              <div style={{ position: "relative" }}>
                <button onClick={() => copy(stage2.python, "stage2")}
                        style={{ ...btn("#d47a3f"), position: "absolute", right: 8, top: 8, zIndex: 1 }}>
      ...
      {allDone && (
        <div style={{ marginTop: 18, padding: "14px 18px", borderRadius: 10, background: "#eef4e8",
                      border: "1px solid #bcd6a6", color: "#cfe6c0", fontWeight: 600 }}>
...
function DeviceCard({ id, snap }: { id: string; snap?: DeviceSnapshot }) {
  ...
  const col = STATUS_COLOR_CSS[state] ?? "#8a7c63";
  ...
  return (
    <div style={{ border: `1px solid ${state === "fault" ? "#eccabf" : "var(--line)"}`, borderRadius: 8,
                  padding: "10px 12px", background: state === "fault" ? "#f7e6e2" : "var(--panel)" }}>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ background: "#f6efe2", border: "1px solid var(--line)", borderRadius: 8, padding: "6px 12px" }}>
...
function CopyLine({ label, text, onCopy, copied }: { label: string; text: string; onCopy: () => void; copied: boolean }) {
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <button onClick={onCopy} style={btn("#e0d0b6", "#5a4c36")}>{copied ? "✓" : "複製"}</button>
    </div>
  );
}
...
function NavCard({ emoji, title, desc, onClick }: { emoji: string; title: string; desc: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ textAlign: "left", cursor: "pointer", background: "var(--panel)",
            ...
            onMouseEnter={(e) => (e.currentTarget.style.borderColor = "#b5622e")}
...
const inp: React.CSSProperties = {
  background: "#f6efe2", color: "#453a29", border: "1px solid #e0d0b6", borderRadius: 6, padding: "6px 10px", minWidth: 180,
};
const card: React.CSSProperties = {
  marginTop: 14, border: "1px solid var(--line)", borderRadius: 10, padding: "14px 16px", background: "var(--panel)",
};
const cardTitle: React.CSSProperties = { fontWeight: 700, color: "#5a4c36", marginBottom: 8 };
const pre: React.CSSProperties = {
  background: "#f6efe2", border: "1px solid var(--line)", borderRadius: 8, padding: "12px 14px",
  overflowX: "auto", fontSize: 12.5, lineHeight: 1.55, margin: 0,
  fontFamily: "'Cascadia Code','Consolas',monospace", color: "#5a4c36",
};

function btn(bg: string, color = "#fffaf0"): React.CSSProperties {
...
function toggleBtn(active: boolean): React.CSSProperties {
  return {
    background: active ? "#b5622e" : "var(--panel2)", color: active ? "#fffaf0" : "var(--text)",
    border: `1px solid ${active ? "#b5622e" : "var(--line)"}`, borderRadius: 6, padding: "5px 12px",
